...
public class RainbowController : MonoBehaviour
{
    ...
    /* Rainbow Credentials */
    [Header("Rainbow Credentials")]
    [Tooltip("Application ID")]
    public string ApplicationID;
    [Tooltip("Application Secret")]
    public string ApplicationSecretKey;
    [Tooltip("Rainbow Server to connect to - usually openrainbow.com or sandbox.openrainbow.com")]
    public string RainbowHostName;
    [Tooltip("Login of your user")]
    public string UserLogin;
    [Tooltip("Password of your user")]
    ...
    private Coroutine NoInputCoroutine = null;
    ...
    private void checkMandatoryAttributes()
    {
        ...
        {
            Assert.IsFalse(String.IsNullOrEmpty(UserLogin), "No QRcodeScanner: UserLogin attribute is mandatory.");
            Assert.IsFalse(String.IsNullOrEmpty(UserPassword), "No QRcodeScanner: UserPassword attribute is mandatory.");
            Assert.IsFalse(String.IsNullOrEmpty(ApplicationID), "No QRcodeScanner: ApplicationID attribute is mandatory.");
            Assert.IsFalse(String.IsNullOrEmpty(ApplicationSecretKey), "No QRcodeScanner: ApplicationSecretKey attribute is mandatory.");
            Assert.IsFalse(String.IsNullOrEmpty(RainbowHostName), "No QRcodeScanner: RainbowHostName attribute is mandatory.");
    ...
    void Start()
    {
        ...
        if (NoKeyboard)
        {
            NoInputCoroutine = StartCoroutine(NoInput());
        }
    }
    ...
    void Update()
    {
        ...
        {
            AddMediaToCurrentCall(Media.VIDEO);
        }
        if (Input.GetKeyDown(KeyCode.S))
        {
            AddMediaToCurrentCall(Media.SHARING);
        }

        if (Input.GetKeyDown(KeyCode.B))
        {
            RemoveMediaFromCurrentCall(Media.VIDEO);
        }
        if (Input.GetKeyDown(KeyCode.D))
        {
            RemoveMediaFromCurrentCall(Media.SHARING);
        }
    ...
    private bool AreCredentialsComplete()
    {
        return (UserLogin.Length > 0 && UserPassword.Length > 0 && ApplicationID.Length > 0 && RainbowHostName.Length > 0 && ApplicationSecretKey.Length > 0);
    }
    ...
    private void StartRainbowWithCredentials(string login, string password, string hostname, string appId, string appSecret)
    {
        ...
        RainbowHostName = hostname;
        ApplicationID = appId;
        ApplicationSecretKey = appSecret;
        ...
        rbApplication.SetApplicationInfo(ApplicationID, ApplicationSecretKey);
        rbApplication.SetHostInfo(RainbowHostName);
        ...
        // Login
        rbApplication.Login(UserLogin, UserPassword, result =>
        {
    ...
    private void SubscribeToMediaPublicationWithRetry(MediaPublication mediaPublication, Action<SdkResult<bool>> cb, int nbTry=0, int maxTry=3 ) {
        rbWebRTCCommunications.SubscribeToMediaPublication(mediaPublication, Conference.SubStreamLevel.NONE, result =>
        {
            if (result.Result.Success)
            {
                cb(result);
    ...
    private void WebRTCCommunications_OnMediaPublicationUpdated(object sender, MediaPublicationEventArgs e)
    {
        ...
        if (e.Status == MediaPublicationStatus.PEER_STARTED)
        {
            ...
            Debug.LogError($"SubscribeToMediaPublication");
